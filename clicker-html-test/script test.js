...
function addClick() {
    mainElementContainer1.addEventListener("click", clickMe);
    mainElementContainer2.addEventListener("click", clickMe);
    mainElementContainer3.addEventListener("click", clickMe);
    mainElementContainer4.addEventListener("click", clickMe);
...
function clickMe() {
    ...
    mainElement.removeEventListener("click", goneMe);
    mainElement.classList.add("paused");
    mainElement.querySelector("img").classList.add("dissolve");
    // mainElementGif1.classList.remove("hidden");
    mainElement.addEventListener("animationend", goneMe);
    incrementPoints();
}
...
function goneMe() {
    const mainElement = this;
    mainElement.removeEventListener("animationend", goneMe)
    mainElement.querySelector("img").classList.remove("dissolve");
    mainElement.classList.remove("paused");
    mainElementRestart.call(this);
    mainElement.addEventListener("click", clickMe);
    mainElementGif1.classList.add("hidden");



    // mainElement.classList.remove("move_center1");
    // mainElement.offsetWidth;
    // mainElementGif1.classList.add("hidden");
    // mainElement.classList.add("move_center1");
    // mainElement.addEventListener("click", clickMe1);
}
function mainElementRestart(){
    console.log("ME restart");
    const mainElement = this;
    mainElement.classList.remove("move_center1");
    mainElement.offsetWidth;
    mainElement.classList.add("move_center1");
    mainElement.classList.remove("position1", "position2", "position3", "position4");

    const p = Math.floor(Math.random() * 4) + 1;
    mainElement.add(`position${p}`);
}
